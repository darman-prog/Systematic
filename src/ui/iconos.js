...
const TRAZOS = {
  ...
  borrar: '<polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>',
  ...
  ordenar: '<line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/>',
...
export function icono(nombre, clases = "", opciones = {}) {
  const clave = ALIAS[nombre] || nombre;
  const trazos = TRAZOS[clave];
  
  if (!trazos) return "";
  
  const cacheKey = clave + "|" + clases + "|" + JSON.stringify(opciones);
  if (cache.has(cacheKey)) return cache.get(cacheKey);
  
  const { size = null, filled = false } = opciones;
  const clasesArray = ["icono"];
  
  if (clases) clasesArray.push(clases);
  if (filled) clasesArray.push("icono-filled");
  if (size) clasesArray.push("icono-" + size);
  
  const svg = '<svg class="' + clasesArray.join(" ") + '" viewBox="0 0 24 24" fill="' + 
...
export function obtenerNombresIconos() {
  return Object.keys(TRAZOS);
}

export function obtenerAlias(nombre) {
